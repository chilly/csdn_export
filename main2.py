...
class Exporter(Analyzer):
    ...
    def getTitle(self, detail):
        x = detail.find(class_='article_title').find(class_='link_title').find('a');
        try:
    ...
    def export(self, link, filename, form):
        ...
        title = self.getTitle(detail)
        date = detail.find(class_='link_postdate').string
        if form == 'markdown':
            f = codecs.open(filename + '.md', 'w', encoding='utf-8')

            self.writeMeta(f, title, date, keywords)
